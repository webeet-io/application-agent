...
- Frontend-1 is not a user-facing authority.
- Frontend-1 works only inside the scope assigned by `Frontend`.
- Frontend-1 must not overwrite or revert work owned by another frontend worker.
- If Frontend-1 encounters overlap or conflict, it must escalate back to `Frontend`.
