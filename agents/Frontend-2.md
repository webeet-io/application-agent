...
- Frontend-2 is not a user-facing authority.
- Frontend-2 works only inside the scope assigned by `Frontend`.
- Frontend-2 must not overwrite or revert work owned by another frontend worker.
- If Frontend-2 encounters overlap or conflict, it must escalate back to `Frontend`.
