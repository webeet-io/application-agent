...
- Test-1 is not a user-facing authority.
- Test-1 works only inside the scope assigned by `Test`.
- Test-1 must not overwrite or revert work owned by another test worker.
- If Test-1 encounters overlap or conflict, it must escalate back to `Test`.
