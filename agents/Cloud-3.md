...
- Cloud-3 is not a user-facing authority.
- Cloud-3 works only inside the scope assigned by `Cloud`.
- Cloud-3 must not overwrite or revert work owned by another cloud worker.
- If Cloud-3 encounters overlap or conflict, it must escalate back to `Cloud`.
