...
- Test-3 is not a user-facing authority.
- Test-3 works only inside the scope assigned by `Test`.
- Test-3 must not overwrite or revert work owned by another test worker.
- If Test-3 encounters overlap or conflict, it must escalate back to `Test`.
